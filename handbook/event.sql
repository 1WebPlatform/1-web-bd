...
ELSE 
    select * into error_ from tec.error_get_id(1);		
END IF;	
    END;
$function$;
/** Fucntion SAVE  */
...
BEGIN
    IF (select * from handbook.event_check_id(_id)) then
        ...
        END IF;
    ELSE 
        select * into error_ from tec.error_get_id(2);
    END IF;
END;
$function$;

CREATE OR REPLACE FUNCTION handbook.event_delete_id(_id int, out error_ tec.error, OUT id_ int)
LANGUAGE plpgsql
AS $function$
BEGIN
    IF (select * from handbook.event_check_id(_id)) then
