...
CREATE OR REPLACE FUNCTION handbook.type_params_check_update(_id int, _name varchar)
RETURNS boolean
LANGUAGE plpgsql
AS $function$
    BEGIN
        return EXISTS (select * from handbook.type_params where id != _id and "name" = _name);
    END;
$function$;
/** Fucntion CHECK  */
...
ELSE 
    select * into error_ from tec.error_get_id(3);		
END IF;	
    END;
$function$;
/** Fucntion SAVE  */
...
BEGIN
    IF (select * from handbook.type_params_check_id(_id)) then
        IF (select * from handbook.type_params_check_update(_id, _name)) <> true then
        ...
        END IF;
    ELSE 
        select * into error_ from tec.error_get_id(4);
    END IF;
END;
$function$;

CREATE OR REPLACE FUNCTION handbook.type_params_delete_id(_id int, out error_ tec.error, OUT id_ int)
LANGUAGE plpgsql
AS $function$
BEGIN
    IF (select * from handbook.type_params_check_id(_id)) then
